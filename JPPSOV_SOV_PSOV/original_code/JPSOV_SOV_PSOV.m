...
%          x=(x-min(x))/(max(x)-min(x));
        ...
        for kk=2:N,
               d(kk)=d(kk-1)/(1+d(kk-1).^2)+x(kk).^2;
        end
   
        snr=20;
        info_power=(norm(d)^2)/N;                    
        snr_lin=10^(snr/10);                         
...
%         lante1=lt2;
        ...
        for i=m1:1:N %输入信号x的长度
            ...
            sv=[1 new_xv].';  %最终得到的volterra扩展的输入值
            ...
            for j=M:-1:1,  %模块的长度
                ...
                external_s=[r(j) x(i-j:-1:i-(j+p-1))];
                    %%%%%%%%%%% Volterra expansion  
                    Xpre=external_s;
                    L=p+1;
                    %%%%%%%%%%%%%%%%%%%%%%%%%
...
%
plot( 1:5000, 10*log10(avg_ev), 'b',1:5000, 10*log10(avg_ep), 'r',1:5000, 10*log10(avg_e_linear), 'g');
